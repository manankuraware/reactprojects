...
export const rating = async (req, res) => {
  try {
    ...
    if (!itemId || !rating) {
      return res.status(400).json({ message: "itemId and raing is required" });
    }
    if (rating < 1 && rating > 5) {
      return res.status(400).json({ message: "Rating is not coorect" });
    }
    ...
    return res.status(500).json({ message: `Items Rating Error ${error}` })
  }
}